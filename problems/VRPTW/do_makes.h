...
eoCheckPoint<EOT>& do_make_checkpoint(eoParser& _parser, eoState& _state, eoContinue<EOT>& _continue)
{
    ...
    eoValueParam<bool>& useEvalParam = _parser.createParam(false, "useEval", "Use nb of eval. as counter (vs nb of gen.)", '\0', "Output");
    eoValueParam<bool>& useTimeParam = _parser.createParam(false, "useTime", "Display time (s) every generation", '\0', "Output");
...
eoContinue<moeoVRP> & do_make_continue(eoParser& _parser, eoState& _state)
{
    ...
    eoValueParam<unsigned long> maxGenParam = _parser.getORcreateParam<unsigned long>(long(100), "maxGen", "Maximum number of generations");
...
eoGenOp<moeoVRP> & do_make_op(eoParser& _parser, eoState& _state, std::string _opt = "")
{
  ...
  eoOpContainer<moeoVRP>* op;
  if (_opt == "CODISEO")
     op = new eoSingleOp<moeoVRP>;
  else
     op = new eoSequentialOp<moeoVRP>;
...
{
   return new moeoParetoObjectiveVectorComparator<tOV>;
}
